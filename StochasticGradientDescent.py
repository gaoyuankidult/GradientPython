"""
This file implements basic stochastic gradient descent
"""
...
for i in range(50000):
    ...
    old_cost = new_cost
    # stochastic gradient
    random_index = [[o] for o in xrange(len(x))]
    shuffle(random_index)
    random_index = [k[0] for k in random_index]
    for j in random_index:
        predicted_value = predict_fun(beta0, beta1, x)
        temp0 = beta0 - learning_rate * (predicted_value[j] - y[j])
        temp1 = beta1 - learning_rate * (predicted_value[j] - y[j]) * x[j]

        # update coefficient
        beta0 = float(temp0)
        beta1 = float(temp1)
        new_cost = cost_fun(y, predict_fun(beta0, beta1, x))
    print "iteration number: %d. Cost now is %f." % (i + 1, new_cost)
